...
import digu_dev.com.github.SchoolAPI.dto.UserResponseDto;
...
public class UserController {
    ...
    @GetMapping("/{username}")
    public ResponseEntity<UserResponseDto> findLoginByUsername(@PathVariable("username") String username) {
        Optional<UserEntity> existingUser = userService.findByUsername(username);
    ...
    public ResponseEntity<List<UserResponseDto>> getUsers() {
        ...
        if(users.isEmpty()){
            ...
            return ResponseEntity.ok(users.stream().map(UserResponseDto::fromEntity).toList()); 
        }  
    }
